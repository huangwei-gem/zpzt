...
const InterviewsList: React.FC = () => {
  ...
  // 可编辑面试官
  const [editingInterviewer, setEditingInterviewer] = useState<{ id: string; field: 'primary_interviewer' | 'secondary_interviewer' } | null>(null);
  const [editValue, setEditValue] = useState('');

  const handleSaveInterviewer = async (record: MergedRow) => {
    if (!editingInterviewer || !record.interview_id) return;
    try {
      await request.put(`/interviews/${record.interview_id}`, {
        [editingInterviewer.field]: editValue,
      });
            ...
            <span>
              ...
              {isEditingPrimary ? (
                <Input
                  ...
                  onBlur={() => handleSaveInterviewer(r)}
                  onPressEnter={() => handleSaveInterviewer(r)}
                  autoFocus
                />
              ) : (
